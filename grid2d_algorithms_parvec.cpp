...
const double grid2d_algorithms_parvec::SPEEDS_X[DIRECTIONS_2D] = { -C, 0, C, -C,
    0, C, -C, 0, C };
const double grid2d_algorithms_parvec::SPEEDS_Y[DIRECTIONS_2D] = { -C, -C, -C,
    0, 0, 0, C, C, C };
const double grid2d_algorithms_parvec::weights[DIRECTIONS_2D] = { 1.0 / 36.0,
    1.0 / 9.0, 1.0 / 36.0, 1.0 / 9.0, 4.0 / 9.0, 1.0 / 9.0, 1.0 / 36.0, 1.0
        / 9.0, 1.0 / 36.0 };

grid2d_algorithms_parvec grid2d_algorithms_parvec::from_file(
...
void grid2d_algorithms_parvec::get_momentum_density(size_t x, size_t y,
    double (&momentum_density)[2]) {
  momentum_density[0] = 0.0;
  momentum_density[1] = 0.0;
  for (size_t dir = 0; dir < DIRECTIONS_2D; dir++) {
    momentum_density[0] += get_population(x, y, dir)
        * grid2d_algorithms_parvec::SPEEDS_X[dir];
  }
  for (size_t dir = 0; dir < DIRECTIONS_2D; dir++) {
    momentum_density[1] += get_population(x, y, dir)
        * grid2d_algorithms_parvec::SPEEDS_Y[dir];
  }
}
...
//  });
  ...
  hpx::parallel::for_each(hpx::parallel::datapar_execution, zip_it_begin,
      zip_it_end, [this](auto t) {

//    using var_type = typename hpx::util::decay<decltype(t)>::type;
//    using no_ref = typename hpx::util::decay<var_type>::type;
    typename hpx::util::decay<decltype(t)>::type v = hpx::util::get<0>(t);
  });
//
//    var_type v;
//    var_type my_var;

//        auto mass_density = get_mass_density(t);

//        //TODO: cannot do this, because there is also a scalar instantiation
//      Vc::double_v cell_pop[9];
//      lattice::util::unpack_tuple<0, 9>(t, cell_pop);

//    Vc::double_v mass_density = get_mass_density(cell_pop);
...
void grid2d_algorithms_parvec::initialize_cell(size_t x, size_t y,
    double factor) {
  for (size_t dir = 0; dir < DIRECTIONS_2D; dir++) {
...
double &grid2d_algorithms_parvec::get_population(int64_t x, int64_t y,
    size_t dir) {
  return (*populations)[dir].at(get_pop_index(x, y));
}

double &grid2d_algorithms_parvec::get_new_population(int64_t x, int64_t y,
    size_t dir) {
