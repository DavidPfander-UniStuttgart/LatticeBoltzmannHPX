...
const double grid2d_algorithms::SPEEDS_X[DIRECTIONS_2D] = { -C, 0, C, -C, 0, C,
    -C, 0, C };
const double grid2d_algorithms::SPEEDS_Y[DIRECTIONS_2D] = { -C, -C, -C, 0, 0, 0,
    C, C, C };
const double grid2d_algorithms::weights[DIRECTIONS_2D] = { 1.0 / 36.0, 1.0
    / 9.0, 1.0 / 36.0, 1.0 / 9.0, 4.0 / 9.0, 1.0 / 9.0, 1.0 / 36.0, 1.0 / 9.0,
    1.0 / 36.0 };

grid2d_algorithms grid2d_algorithms::from_file(std::string file_name,
    bool verbose) {
  std::stringstream ss;
...
void grid2d_algorithms::serialize_as_csv(const std::string &file_name) {
  ...
  myfile << "x" << sep << "y" << sep << "z (dummy)" << sep << "value"
      << std::endl;
  for (size_t x = 0; x < x_size; x++) {
    for (size_t y = 0; y < y_size; y++) {
...
grid2d_algorithms::grid2d_algorithms(size_t x_size, size_t y_size,
    std::vector<lattice::CELL_TYPES> &cells_unpadded, bool verbose) :
    verbose(verbose), x_size(x_size), y_size(y_size) {
  populations =
      std::make_unique<std::array<std::vector<double>, DIRECTIONS_2D>>();
  new_populations = std::make_unique<
      std::array<std::vector<double>, DIRECTIONS_2D>>();
  ...
  for (size_t dir = 0; dir < DIRECTIONS_2D; dir++) {
    ...
    std::fill((*new_populations)[dir].begin(), (*new_populations)[dir].end(),
        0.0);
  }
  ...
  for (int64_t x = -1; x < static_cast<int64_t>(x_size + 1); x++) {
    for (int64_t y = -1; y < static_cast<int64_t>(y_size + 1); y++) {
      if (x >= 0 && x < static_cast<int64_t>(x_size) && y >= 0
          && y < static_cast<int64_t>(y_size)) {
...
void grid2d_algorithms::get_momentum_density(
    hpx::util::tuple<double &, double &, double &, double &, double &, double &,
        double &, double &, double &, CELL_TYPES> &t,
    double (&momentum_density)[2]) {
...
void grid2d_algorithms::get_momentum_density(double (&cell_pop)[9],
    double (&momentum_density)[2]) {
  ...
  for (size_t dir = 0; dir < DIRECTIONS_2D; dir++) {
    momentum_density[0] += cell_pop[dir] * grid2d_algorithms::SPEEDS_X[dir];
  }

  for (size_t dir = 0; dir < DIRECTIONS_2D; dir++) {
    momentum_density[1] += cell_pop[dir] * grid2d_algorithms::SPEEDS_Y[dir];
  }
}
...
double grid2d_algorithms::get_mass_density(double (&cell_pop)[9]) {
  double mass_density = 0.0;
  for (size_t dir = 0; dir < DIRECTIONS_2D; dir++) {
...
void grid2d_algorithms::get_momentum_density(size_t x, size_t y,
    double (&momentum_density)[2]) {
  momentum_density[0] = 0.0;
  momentum_density[1] = 0.0;
  for (size_t dir = 0; dir < DIRECTIONS_2D; dir++) {
    momentum_density[0] += get_population(x, y, dir)
        * grid2d_algorithms::SPEEDS_X[dir];
  }
  for (size_t dir = 0; dir < DIRECTIONS_2D; dir++) {
    momentum_density[1] += get_population(x, y, dir)
        * grid2d_algorithms::SPEEDS_Y[dir];
  }
}
...
double grid2d_algorithms::calculate_equilibrium(const size_t dir,
    const double mass_density, const double (&u)[2]) {
...
void grid2d_algorithms::collide() {
  ...
  auto zip_it_begin = hpx::util::make_zip_iterator(
      boost::begin((*populations)[0]), boost::begin((*populations)[1]),
      boost::begin((*populations)[2]), boost::begin((*populations)[3]),
      boost::begin((*populations)[4]), boost::begin((*populations)[5]),
      boost::begin((*populations)[6]), boost::begin((*populations)[7]),
      boost::begin((*populations)[8]), boost::begin(cells));

  auto zip_it_end = hpx::util::make_zip_iterator(boost::end((*populations)[0]),
      boost::end((*populations)[1]), boost::end((*populations)[2]),
      boost::end((*populations)[3]), boost::end((*populations)[4]),
      boost::end((*populations)[5]), boost::end((*populations)[6]),
      boost::end((*populations)[7]), boost::end((*populations)[8]),
      boost::end(cells));
...
//          double mass_density = get_mass_density(t);
          double mass_density = get_mass_density(cell_pop);
          double momentum_density[2];
...
void grid2d_algorithms::source() {
  ...
  index_iterator::iterate_indices<2>(policy, { 0, 0 }, { x_size, y_size },
...
double &grid2d_algorithms::get_new_population(int64_t x, int64_t y,
    size_t dir) {
