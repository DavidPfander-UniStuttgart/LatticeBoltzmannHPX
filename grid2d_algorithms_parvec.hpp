...
public:
  ...
  static grid2d_algorithms_parvec from_file(std::string file_name,
      bool verbose);
...
//    std::vector<double> new_populations[DIRECTIONS_2D];
  ...
  grid2d_algorithms_parvec(size_t x_size, size_t y_size,
...
//    void velocity_to_momentum(const double (&v)[9], const double mass_density, double (&momentum)[9]);
  ...
  void get_momentum_density(T (&cell_pop)[9], T (&momentum_density)[2]) {
    ...
    for (size_t dir = 0; dir < DIRECTIONS_2D; dir++) {
      momentum_density[0] += cell_pop[dir]
          * grid2d_algorithms_parvec::SPEEDS_X[dir];
    }

    for (size_t dir = 0; dir < DIRECTIONS_2D; dir++) {
      momentum_density[1] += cell_pop[dir]
  ...
  template<typename T>
  double calculate_equilibrium(const size_t dir, const T mass_density,
      const T (&u)[2]) {
...
//  void collide_cells(
//      hpx::util::tuple<
//          Vc::double_v &, Vc::double_v &, Vc::double_v &,
//          Vc::double_v &, Vc::double_v &, Vc::double_v &,
//          Vc::double_v &, Vc::double_v &, Vc::double_v &> t) {

  void collide_cells(Vc::double_v d) {
...
//                * calculate_equilibrium(8, mass_density, u));
  ...
  typename hpx::util::decay<Tuple>::type get_mass_density(Tuple &t) {
